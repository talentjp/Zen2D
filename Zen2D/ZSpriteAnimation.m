...
@implementation ZSpriteAnimation
...
- (void)loadSpritesFromFilenames:(NSString *)firstFile, ...
{
    ...
    if(firstFile)
    {
        ZSprite* newSprite = [[ZSprite alloc] initWithFile:firstFile];
        [self attachNode:newSprite];
        va_start(argumentList, firstFile);
        while((eachFile = va_arg(argumentList, NSString*)))
        {
            newSprite = [[ZSprite alloc] initWithFile:eachFile];
            [self attachNode:newSprite];
        }
        va_end(argumentList);
...
- (void)loadSpritesFromArrayOfFiles:(NSArray *)arrayFiles
{
    if(arrayFiles)
    {
        ...
        {
            ZSprite* newSprite = [[ZSprite alloc] initWithFile:filename];
            [self attachNode:newSprite];
        }
        [self setActiveSpriteByIndex:0];
    }
...
{
    for(ZSprite* sprite in self.arrayChildren)
    {
        sprite.hide = YES;
    }
    
    ZSprite* activeSprite = [self.arrayChildren objectAtIndex:index];
