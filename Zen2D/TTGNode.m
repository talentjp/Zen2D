...
- (void)addComponent:(TTGComponent*)component
{
...
{
    TTGAnimator* animatorToRemove = nil;
...
- (void)addNodeToSceneForRender:(TTGScene *)scene
{
    if(!self.hide)
    {
        if([self isKindOfClass:[TTGSprite class]])
...
{
    for(TTGComponent* comp in _arrayComponents)
    {
        [comp reset];
    }
}
...
- (void)updateWithTime:(float)deltaTime
{
    ...
    for(TTGAnimator* animator in tempArray)
    {
...
{
    TTGNode* newNode = [[[self class] allocWithZone:zone] init];
    if(newNode)
    {
...
{
    for(TTGComponent* component in _arrayComponents)
    {
        [component activate];
    }
}
