...
@implementation TTGRigidbodyComponent
...
- (void)cleanup
{
    if(_body)
    {
        [[TTGPhysicsManager sharedManager] getWorld]->DestroyBody(_body);
        _body = nil;
    }
    [[TTGPhysicsManager sharedManager] removeComponent:self];
}
...
- (void)computeFixtureDefForJSON
{
    ...
    for(int i = 0; i < vertex_count; i++)
    {
        ...
        x = x * scaleH/ [[TTGPhysicsManager sharedManager] getMeterPixelRatio];
        float y = [[[arrayPath objectAtIndex:i] objectForKey:@"Y"] floatValue];
        y = y * scaleV/ [[TTGPhysicsManager sharedManager] getMeterPixelRatio];
        vertices[i].Set(x, y);
    }
...
{
    if(![_parentNode isKindOfClass:[TTGSprite class]])
    {
        NSException* exception = [NSException exceptionWithName:@"PhysicsSetupException"
                                                         reason:@"Cannot infer the dimensions from a non-TTGSprite object"
                                                       userInfo:nil];
        @throw exception;
    }
    else
    {
        float width = ((TTGSprite*)_parentNode).spriteSize.width * ((TTGSprite*)_parentNode).scale.width / [TTGPhysicsManager sharedManager].meterPixelRatio / 2.0;
        float height = ((TTGSprite*)_parentNode).spriteSize.height * ((TTGSprite*)_parentNode).scale.height / [TTGPhysicsManager sharedManager].meterPixelRatio  / 2.0;
...
- (void)computeFixtureDefForCircle
{
    ...
    {
        NSException* exception = [NSException exceptionWithName:@"PhysicsSetupException"
                                                         reason:@"Cannot infer the radius from a non-TTGSprite object"
                                                       userInfo:nil];
        @throw exception;
    }
    else
...
- (void)attachedToNode
{
    ...
    [[TTGPhysicsManager sharedManager] addRigidbodyComponent:self];
...
- (void)recomputeBody
{
    ...
    float startX = self.parentNode.spritePosition.x / [[TTGPhysicsManager sharedManager] getMeterPixelRatio];
    float startY = self.parentNode.spritePosition.y / [[TTGPhysicsManager sharedManager] getMeterPixelRatio];
    _bodyDef.type = _bodyType;
    _bodyDef.position.Set(startX, startY);
    _bodyDef.angle = self.parentNode.rotation / 180.0 * 3.1415926;
    _body = [[TTGPhysicsManager sharedManager] getWorld]->CreateBody(&_bodyDef);
...
- (void)update
{
    if(_body)
    {
        ...
        float meterPixelRatio = [[TTGPhysicsManager sharedManager] getMeterPixelRatio];
        [self.parentNode moveToX:position.x * meterPixelRatio  Y:position.y * meterPixelRatio];
        ((TTGNode*)self.parentNode).rotation = angle * 180.0 / 3.1415926;
    }
}
...
{
    TTGRigidbodyComponent* newComp = [super copyWithZone:zone];
    if(newComp)
    {
