...
static TTGTextureManager* singleton = nil;

@implementation TTGTextureManager
...
+ (TTGTextureManager *)sharedManager
{
    @synchronized(singleton)
    {
        if(!singleton)
        {
            singleton = [[TTGTextureManager alloc] init];
        }
    }
    return singleton;
}
...
- (void)loadJSONSpriteSheet:(NSString *)sheetName
{
    ...
    if(jsonError)
    {
        ...
        for(id jsonElement in jsonArray)
        {
            ...
            TTGTexture* newTexture = [[TTGTexture alloc] initWithTextureInfo:textureInfo];
