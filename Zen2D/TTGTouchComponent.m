...
@implementation TTGTouchComponent
...
- (void)touchBegan:(UITouch *)touch withEvent:(UIEvent *)event
{
    ...
    {
        CGPoint touchLocation = [[TTGTouchManager sharedManager] getTouchLocation:touch inScene:_parentNode.parentScene];
...
- (float)getParentDepth
{
    ...
    {
        return ((TTGSprite*)_parentNode).spriteDepth;
    }
    return 0;
}
...
- (void)absorbTouch
{
    [[TTGTouchManager sharedManager] cancelTouch];
}

- (void)cleanup
{
    [[TTGTouchManager sharedManager] removeComponent:self];
}
...
{
    [super activate];
    [[TTGTouchManager sharedManager] addTouchComponent:self];
}
